    ...
    : AudioProcessorEditor (&p), audioProcessor (p)
{
    ...
    */
    ...
    versionLabel.setText("v0.03.240703", juce::dontSendNotification);
...
void RhythmPatternExplorerAudioProcessorEditor::timerCallback()
{
    ...
    // Debug animation state every 30 frames (~0.5 seconds at 60fps)
    if (frameCount % 30 == 0)
    {
        DBG("Timer callback - Step: " + juce::String(currentStep) + 
            ", Playing: " + juce::String(isPlaying ? "true" : "false") + 
            ", Hash: " + juce::String(currentHash));
    }
    ...
    if (currentHash != lastUpdateHash)
    {
        ...
        updateAnalysisDisplay();
        shouldRepaint = true;
        DBG("Pattern changed - forcing repaint");
    }
    
    if (currentStep != lastCurrentStep)
    {
        shouldRepaint = true;
        DBG("Current step changed from " + juce::String(lastCurrentStep) + " to " + juce::String(currentStep));
    }
...
void RhythmPatternExplorerAudioProcessorEditor::drawPatternCircle(juce::Graphics& g, juce::Rectangle<int> bounds)
{
    ...
    float innerRadius = maxRadius * 0.2f;  // Slightly larger hole for better visibility
    ...
    // Enable debug for animation troubleshooting
    DBG("Current step: " + juce::String(currentStep) + ", Playing: " + juce::String(isPlaying ? "true" : "false") + ", Pattern size: " + juce::String(numSteps));
    ...
    // Alternative approach: Draw using basic geometry instead of addPieSegment
    for (int i = 0; i < numSteps; ++i)
    {
        ...
        if (pattern[i])
        {
            ...
            slice.addArc(center.x - outerRadius, center.y - outerRadius, 
            ...
            {
                juce::Path innerHole;
                innerHole.addEllipse(center.x - innerRadius, center.y - innerRadius, 
                                   innerRadius * 2, innerRadius * 2);
                // Remove inner hole using path subtraction
                slice.setUsingNonZeroWinding(false);
                slice.addPath(innerHole);
            }
            
            g.setColour(juce::Colour(0xff48bb78));  // Green for onsets
            g.fillPath(slice);
            
            DBG("Drew onset slice " + juce::String(i) + " from " + juce::String(startAngle) + " to " + juce::String(endAngle));
        }
    }
    ...
    if (isPlaying && currentStep >= 0 && currentStep < numSteps)
    {
        ...
        // Create arc for the outer edge
        highlightSlice.addArc(center.x - outerRadius, center.y - outerRadius, 
                             outerRadius * 2, outerRadius * 2,
                             startAngle, endAngle, true);
        ...
        if (innerRadius > 0)
        {
            ...
            innerHole.addEllipse(center.x - innerRadius, center.y - innerRadius, 
                               innerRadius * 2, innerRadius * 2);
            // Remove inner hole using path subtraction
            highlightSlice.setUsingNonZeroWinding(false);
            highlightSlice.addPath(innerHole);
        }
        
        g.setColour(juce::Colour(0xffff6b35));  // Bright orange highlight
        g.fillPath(highlightSlice);
        
        DBG("Highlighting step " + juce::String(currentStep) + " from " + juce::String(startAngle) + " to " + juce::String(endAngle));
    }
